...
*Figure 3: Dual-panel clinical explainability dashboard generated by Netra Rakshak: (Left) High-resolution input fundus scan (True Level 4: Proliferative Diabetic Retinopathy); (Right) Clinically evaluated lesion focus map via Grad-CAM thermal overlay with quantitative colorbar spectrum and automated status annotation bar.*

#### Mathematical Formulation of Visual Explanations
To eliminate black-box opacity and provide actionable decision provenance for reviewing ophthalmologists, Netra Rakshak implements Gradient-weighted Class Activation Mapping (Grad-CAM). Gradients of the predicted class score $y^c$ (before softmax normalization) are computed with respect to the feature activation maps $A^k$ of the final residual bottleneck convolution layer (`activation_49_relu`):
$$\alpha_k^c = \frac{1}{Z} \sum_{i=1}^{U} \sum_{j=1}^{V} \frac{\partial y^c}{\partial A_{i,j}^k}$$
Where:
- $Z = U \times V$ represents the spatial dimensions ($7 \times 7$) of the bottleneck feature map
- $\alpha_k^c$ captures the importance weight of feature map $k$ for target diagnostic class $c$
